...
export default (initialLifts) => {
  ...
  return {
    ...
    editLift: (liftId, updatedName) => {
      const liftIsDuplicate = checkForDuplicate(lifts, 'liftName', updatedName);
      if (!updatedName || liftIsDuplicate) return;
      const originalName = lifts.find((lift) => lift.id === liftId).liftName;
      setLifts(
        alphabetizeLifts(
          lifts.map((lift) =>
            lift.id === liftId ? { ...lift, liftName: updatedName } : lift
          )
        )
      );
